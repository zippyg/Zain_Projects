...
def get_S_chi(vals, mass_range, nbins, A, lamb):

    bin_heights, bin_edges = np.histogram(vals, range = mass_range, bins = nbins)
    half_bin_width = 0.5*(bin_edges[1] - bin_edges[0])
    ys_expected = get_B_expectation(bin_edges + half_bin_width, A, lamb)+signal_gaus(bin_edges + half_bin_width, 125, 1.5, 700)
    chi = 0
